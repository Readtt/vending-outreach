...
export function InboxView({ threads }: InboxViewProps) {
  ...
  const [selectedId, setSelectedId] = useState<string | null>(threads[0]?.leadId ?? null)
    ...
    return (
      <div className="rounded-xl border border-dashed border-border px-6 py-16 text-center">
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Every reply the bot could handle on its own, it did — this list only ever holds the
          ones it couldn&apos;t.
        </p>
      </div>
    )
  ...
  function handleOutcome(leadId: string, outcome: ThreadOutcome, label: string) {
    setActingId(leadId)
    startTransition(() => {
  ...
  const originalSubject = selected.messages.find((m) => m.subject)?.subject ?? selected.name
    ...
    <div className="grid gap-4 lg:grid-cols-[minmax(0,300px)_1fr]">
      ...
      <div className="flex flex-col gap-4 rounded-xl border border-border p-4">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <h2 className="text-base font-medium">{selected.name ?? "Unnamed business"}</h2>
            <p className="text-xs text-muted-foreground">
              {[selected.type, selected.phone, selected.email].filter(Boolean).join(" · ") ||
        ...
        <div className="flex max-h-96 flex-col gap-2 overflow-y-auto">
          {selected.messages.length === 0 ? (
            <p className="text-sm text-muted-foreground">No messages on file.</p>
          ) : (
        ...
        <div className="flex flex-wrap items-center gap-2 border-t border-border pt-3">
          <Button size="sm" disabled={busy} onClick={() => handleOutcome(selected.leadId, "won", "won")}>
            Mark won
          </Button>
          <Button
            size="sm"
            variant="outline"
            disabled={busy}
            onClick={() => handleOutcome(selected.leadId, "nurture", "back to nurture")}
          >
            Back to nurture
          </Button>
          <Button
